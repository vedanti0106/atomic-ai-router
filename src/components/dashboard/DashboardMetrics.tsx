  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
      {metrics.map((m, idx) => (
        <div 
          key={idx} 
          className="bg-white rounded-[20px] p-6 border border-line shadow-[0_4px_24px_rgba(15,27,61,0.02)] hover-lift-card cursor-pointer group"
        >
          <div className="flex justify-between items-start mb-4">
            <div className="w-11 h-11 rounded-full bg-sky flex items-center justify-center text-xl text-blue-brand group-hover:scale-110 group-hover:bg-blue-brand group-hover:text-white transition-all duration-300">
              {m.icon}
            </div>
