...
const STATUS_CONFIG: Record<EscrowStatus, { label: string; color: string; bg: string; border: string; dot: string; icon: string }> = {
  FUNDED:   { label: 'Funds Locked',  color: 'text-amber-700',   bg: 'bg-amber-50',   border: 'border-amber-200',  dot: 'bg-amber-500',   icon: '🔒' },
  RELEASED: { label: 'Released',      color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200',dot: 'bg-emerald-500', icon: '✅' },
...
function EscrowCard({ escrow, onAction }: { escrow: EscrowRecord; onAction: (action: string, taskId: string) => void }) {
  ...
  return (
    <div className="bg-white rounded-[20px] border border-line shadow-[0_4px_24px_rgba(15,27,61,0.03)] p-6 flex flex-col gap-4">
      {/* Header row */}
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="font-mono text-[11px] text-blue-brand">{escrow.taskId}</div>
          </div>
        </div>
        <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[12px] font-bold border ${cfg.bg} ${cfg.color} ${cfg.border}`}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-slate-50 rounded-[12px] p-3">
          <div className="text-[11px] text-slate-400 font-medium mb-0.5">Locked Amount</div>
        ...
        <div className="bg-slate-50 rounded-[12px] p-3">
          <div className="text-[11px] text-slate-400 font-medium mb-0.5">Escrow App ID</div>
      ...
      {escrow.status === 'RELEASED' && escrow.proofHash && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-[12px] p-3">
          <div className="text-[11px] font-bold text-emerald-700 mb-1">✅ Delivery Proof (SHA-256)</div>
          <div className="font-mono text-[11px] text-emerald-800 break-all">{escrow.proofHash}</div>
        </div>
      )}
      ...
      <div className="flex flex-col gap-1">
        {escrow.txidFund && (
          <div className="flex items-center gap-2 text-[12px]">
            <span className="text-slate-400 w-14 flex-shrink-0">Fund:</span>
            <span className="font-mono text-blue-brand">{escrow.txidFund}</span>
          </div>
        )}
        {escrow.txidRelease && (
          <div className="flex items-center gap-2 text-[12px]">
            <span className="text-slate-400 w-14 flex-shrink-0">Release:</span>
            <span className="font-mono text-emerald-600">{escrow.txidRelease}</span>
          </div>
        )}
        {escrow.txidRefund && (
          <div className="flex items-center gap-2 text-[12px]">
            <span className="text-slate-400 w-14 flex-shrink-0">Refund:</span>
            <span className="font-mono text-blue-brand">{escrow.txidRefund}</span>
          </div>
        )}
      </div>
      ...
      <div className="pt-2 border-t border-line flex flex-wrap gap-2">
        {escrow.status === 'FUNDED' && (
          <>
            <button
              ...
              className="flex-1 px-3 py-2 bg-emerald-600 text-white text-[12px] font-bold rounded-full hover:bg-emerald-700 transition-colors"
            >
              ✅ Release Funds
            </button>
            <button
              onClick={() => onAction('dispute', escrow.taskId)}
              className="px-3 py-2 bg-rose-50 text-rose-700 border border-rose-200 text-[12px] font-bold rounded-full hover:bg-rose-100 transition-colors"
            >
              ⚠ Dispute
            </button>
            {escrow.canRefundNow && (
              <button
                ...
                className="flex-1 px-3 py-2 bg-blue-brand text-white text-[12px] font-bold rounded-full hover:bg-blue-dark transition-colors"
              >
                ↩ Trigger Refund
              </button>
            )}
        ...
        <button
          ...
          className="px-3 py-2 bg-sky text-blue-brand text-[12px] font-bold rounded-full hover:bg-blue-brand hover:text-white transition-colors"
        >
          🔗 Explorer
        </button>
      </div>
      ...
      <div className="max-w-7xl mx-auto pb-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-bold text-emerald-700">Algorand TestNet — Round {MOCK_CURRENT_ROUND.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total Locked',    value: `$${totalLocked.toFixed(2)}`,    sub: `${counts.FUNDED + counts.DISPUTED} escrows active`,  color: 'text-amber-700',   bg: 'bg-amber-50',   icon: '🔒' },
            { label: 'Total Released',  value: `$${totalReleased.toFixed(2)}`,  sub: `${counts.RELEASED} successful deliveries`,            color: 'text-emerald-700', bg: 'bg-emerald-50', icon: '✅' },
            { label: 'Total Refunded',  value: `$${totalRefunded.toFixed(2)}`,  sub: `${counts.REFUNDED} auto-refunds triggered`,           color: 'text-blue-700',    bg: 'bg-blue-50',    icon: '↩' },
            { label: 'Open Disputes',   value: `${counts.DISPUTED}`,            sub: 'Awaiting admin resolution',                           color: 'text-rose-700',    bg: 'bg-rose-50',    icon: '⚠' },
          ].map((m, i) => (
            <div key={i} className={`rounded-[20px] p-5 border shadow-[0_4px_24px_rgba(15,27,61,0.02)] ${m.bg} border-${m.color.replace('text-','').replace('-700','')}-200`}>
              <div className="text-2xl mb-2">{m.icon}</div>
              <div className={`text-[22px] font-bold font-display ${m.color}`}>{m.value}</div>
        ...
        {/* How it works banner */}
        <div className="bg-white rounded-[20px] border border-line p-6 mb-8 shadow-[0_4px_24px_rgba(15,27,61,0.02)]">
          <div className="flex items-center justify-between mb-5">
            ...
            <span className="text-[11px] font-bold text-blue-brand bg-sky px-3 py-1 rounded-full uppercase">AVM Box Storage</span>
          </div>
          <div className="flex flex-wrap items-center gap-2 text-[12px]">
            {[
              { label: '① fund_escrow', desc: 'Client → Contract', color: 'bg-amber-100 text-amber-800 border-amber-200' },
              { label: '→', desc: '', color: 'bg-transparent text-slate-400 border-transparent' },
              { label: '② release_escrow', desc: 'Facilitator proof ✓', color: 'bg-emerald-100 text-emerald-800 border-emerald-200' },
        ...
        {/* Action log */}
        {actionLog.length > 0 && (
          <div className="bg-slate-900 rounded-[16px] p-4 mb-6 font-mono text-[12px] flex flex-col gap-1.5">
            ...
            {actionLog.map((l, i) => (
              <div key={i} className="flex items-center gap-3 text-slate-300">
                ...
                <span className="text-blue-400 ml-auto">{l.txId}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {/* Filter tabs */}
        <div className="flex items-center gap-1.5 p-1.5 bg-slate-100/80 rounded-[14px] w-fit mb-6">
          {(['ALL', 'FUNDED', 'RELEASED', 'REFUNDED', 'DISPUTED'] as const).map(s => (
            <button
              key={s}
              onClick={() => setFilterStatus(s)}
              className={`px-3 py-1.5 rounded-[10px] text-[12px] font-bold transition-all flex items-center gap-1.5 ${
                filterStatus === s ? 'bg-white text-navy shadow-sm' : 'text-slate-500 hover:text-navy'
              }`}
            >
              {s !== 'ALL' && <span className={`w-1.5 h-1.5 rounded-full ${STATUS_CONFIG[s].dot}`} />}
              {s} <span className="text-[11px] opacity-60">({counts[s as keyof typeof counts] ?? escrows.length})</span>
            </button>
          ))}
        </div>
        ...
        {/* Escrow cards grid */}
        {processing && (
          <div className="mb-4 px-4 py-3 bg-amber-50 border border-amber-200 rounded-[12px] text-[13px] text-amber-800 font-bold flex items-center gap-2">
        ...
        <div className="mt-8 bg-slate-900 rounded-[20px] p-6 text-[12px] font-mono text-slate-400">
          <div className="text-slate-300 font-bold mb-3 text-[13px]">📄 Smart Contract Reference — escrow_contract.py (Beaker + PyTeal)</div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <div key={i} className="flex flex-col gap-0.5">
                <span className={`${m.color}`}>{m.fn}</span>
